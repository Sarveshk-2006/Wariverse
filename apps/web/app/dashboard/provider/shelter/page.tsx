...
export default function ProviderShelterPage() {
    ...
    <div className="dashboard-layout">
      ...
      <main className="dashboard-main">
        <header className="dashboard-header">
          <div>
            <h1 style={{ fontSize: '1.25rem', fontWeight: 800 }}>🏠 My Shelter</h1>
            <p style={{ fontSize: '0.8rem', color: '#6B7280' }}>Service Provider — Shelter & Dharamshala occupancy</p>
          </div>
        ...
        <div className="dashboard-content">
          <div className="stats-grid" style={{ gridTemplateColumns: 'repeat(3, 1fr)', marginBottom: '1.5rem' }}>
            {[
              ...
              { label: 'Available', value: (totalCap - totalOcc).toLocaleString(), color: '#22C55E', icon: '✅' },
            ].map(s => (
                ...
                return (
                  <div key={s.id} className="card" style={{ borderTop: `3px solid ${isFull ? '#EF4444' : pct > 80 ? '#F97316' : '#22C55E'}` }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.75rem' }}>
                      <div><div style={{ fontWeight: 700, fontSize: '0.95rem' }}>{s.name}</div><div style={{ fontSize: '0.75rem', color: '#6B7280' }}>{s.shelter_type}</div></div>
